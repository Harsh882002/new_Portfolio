      ...
      <ul className="w-screen h-1/2     ">
        ...
        {project.map((curElem, key) => (
          <div key={key}  >
            ...
            <h1 className="text-center font-parkinsans mt-4 p-3 text-2xl text-orange-500 font-parkinsans" >{curElem.heading}</h1>
            ...
            <ul className='grid grid-cols-2 mr-8 ml-8 gap-3 mb-4'>
              {curElem.skills.map((skill, index) => (
                <li key={index} className='border-4 font-bold border-orange-500 text-center rounded w-30 '>{skill}</li>
              ))}
            </ul>
            <p>
              <a href={curElem.visitLink} target="_blank" rel="noopener noreferrer" className='ml-3 text-lg '>
                CLICK ME TO VISIT PROJECT
